...
type Paste struct {
    ...
    FileContent *multipart.FileHeader `json:"file"`
}
...
func getPasteById(id string) (*Paste, error) {
    ...
    err := row.Scan(&savedPaste.UUID, &savedPaste.Title, &savedPaste.CreatedAt, &savedPaste.ExpiresAt, &savedPaste.Visibility, &savedPaste.OwnerId)
    if err != nil {
        return nil, err
    }
